import type { Instance, RegisterOptions, Registry } from "../core/types";
...
export class FakeRegistry implements Registry {
  ...
  async watch(service: string, listener: (i: Instance[]) => void, group = "DEFAULT_GROUP") {
    this.watchCalls++;
  ...
  async unwatch(service: string, listener: (i: Instance[]) => void, group = "DEFAULT_GROUP") {
  ...
  async observe(dataId: string, listener: (c: string) => void, group = "DEFAULT_GROUP") {
    const key = `${group}::${dataId}`;
  ...
  async unobserve(dataId: string, listener: (c: string) => void, group = "DEFAULT_GROUP") {
    this.unobserveCalls++;
