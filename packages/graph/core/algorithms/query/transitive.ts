 ...
 * 每行 words 个 32 位字，第 j 位表示是否可达下标 j 的节点。
 */
interface Closure {
 ...
 * 一次性求出全图可达性位图。
 *
 * 先缩点（{@link scc} 按逆拓扑序产出分量），再按该顺序逐个分量做位图或运算——
 * 处理某分量时其所有后继分量已算完，故一遍扫描即可收敛。
 * 复杂度 O(V + E·V/32)，相比「每个节点跑一次 DFS」省掉一个数量级的重复遍历与临时集合。
 */
