...
export function dijkstra<E, G extends Catalog & IntoEdges<E>>(
  ...
  if (isIndexable(graph)) return dense(graph, start, end, edgeCost);
...
function isIndexable(graph: object): graph is NodeIndexable {
  const g = graph as Partial<NodeIndexable>;
  return (
    typeof g.indexOf === "function" &&
    typeof g.at === "function" &&
    typeof g.bound === "function"
  );
}
