...
/** 除冲突边外，每条边都必须满足 rank(source) < rank(target)。 */
const consistent = (
...
describe("增量拓扑序", () => {
  ...
  it("DAG 上的增量顺序与全量拓扑排序等效", () => {
    const graph = randomGraph(61, { order: 25, density: 2, acyclic: true });
    const ordering = new Ordering(graph);
  ...
  it("增删节点后 rank 表跟着变", () => {
    const graph = randomGraph(63, { order: 12, density: 2, acyclic: true });
    const ordering = new Ordering(graph);
  ...
  it("事务里的批量变更在末尾一次消化", () => {
    const graph = randomGraph(64, { order: 15, density: 2, acyclic: true });
    const ordering = new Ordering(graph);
  ...
  it("compare 可直接用作排序比较器", () => {
    const graph = randomGraph(65, { order: 15, density: 2, acyclic: true });
    const ordering = new Ordering(graph);
  ...
  it("refresh 后与全量重算一致", () => {
    const graph = randomGraph(66, { order: 20, density: 2, acyclic: true });
    const ordering = new Ordering(graph);
    ...
    expect(full).toHaveLength(graph.order);
    ordering.dispose();
  });

  it("dispose 之后不再跟随图变化", () => {
    const graph = randomGraph(67, { order: 10, density: 1, acyclic: true });
    const ordering = new Ordering(graph);
    ordering.dispose();
