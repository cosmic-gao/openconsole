...
describe("拓扑序", () => {
  it.each(SEEDS)("seed %i：DAG 的输出满足每条边的先后约束", (seed) => {
    ...
    const order = settle(toposort(Snapshot.of(graph)));
  ...
  it.each(SEEDS)("seed %i：有环图把环节点单列，其余仍是合法拓扑序", (seed) => {
    ...
    const result = settle(topology(Snapshot.of(graph)));

    expect(result.order.length + result.cycle.length).toBe(graph.order);
    const rank = new Map(result.order.map((id, at) => [id, at]));
    for (const edge of graph.edges()) {
      const record = graph.edge(edge)!;
      const from = rank.get(record.source);
      const to = rank.get(record.target);
  ...
  it.each(SEEDS)("seed %i：分层结果同层无依赖，且与拓扑序一致", (seed) => {
    const graph = randomGraph(seed, { order: 30, density: 3, acyclic: true });
    const layers = settle(generations(Snapshot.of(graph)));

    const depth = new Map<NodeId, number>();
    layers.forEach((layer, at) => layer.forEach((id) => depth.set(id, at)));
    expect(depth.size).toBe(graph.order);
    for (const edge of graph.edges()) {
      const record = graph.edge(edge)!;
      expect(depth.get(record.source)!).toBeLessThan(depth.get(record.target)!);
    }
  });
  ...
  it.each(SEEDS)("seed %i：关键路径长度等于路径上的权重和", (seed) => {
    ...
    const result = settle(criticalPath(weighted(graph)));

    let total = 0;
    for (let i = 0; i + 1 < result.path.length; i++) {
      const between = graph.between(result.path[i]!, result.path[i + 1]!);
...
describe("连通性", () => {
  it.each(SEEDS)("seed %i：弱连通分量与逐点扩散一致", (seed) => {
    ...
    const found = settle(components(Snapshot.of(graph))).groups();
    expect(groupKeys(found)).toEqual(setKeys(naiveComponents(graph)));
  });

  it.each(SEEDS)("seed %i：强连通分量与互相可达判定一致", (seed) => {
    const graph = randomGraph(seed, { order: 25, density: 2 });
    const found = settle(scc(Snapshot.of(graph))).groups();
    expect(groupKeys(found)).toEqual(setKeys(naiveScc(graph)));
  });
  ...
  it.each(SEEDS)("seed %i：桥与割点符合删除后分量增加的定义", (seed) => {
    ...
    const result = settle(cuts(Snapshot.of(graph)));

    const found = new Set(result.bridges.map((b) => key(b.from, b.to)));
    expect([...found].sort()).toEqual([...naiveBridges(graph)].sort());
    expect(result.articulations.slice().sort()).toEqual(
  ...
  it.each(SEEDS)("seed %i：支配树与迭代数据流一致", (seed) => {
    ...
    const found = settle(dominators(Snapshot.of(graph), entry));
    const expected = naiveDominators(graph, entry);

    expect(found.size).toBe(expected.size);
    for (const [node, idom] of expected) expect(found.get(node)).toBe(idom);
  });
  ...
  it.each(SEEDS.slice(0, 4))(
    ...
    (seed) => {
      ...
      for (const cycle of settle(simpleCycles(Snapshot.of(graph)))) {
...
describe("可达性", () => {
  it.each(SEEDS)("seed %i：传递闭包与逐点搜索一致", (seed) => {
    ...
    const found = settle(closure(Snapshot.of(graph)));
    const expected = naiveReach(graph);

    for (const node of graph.nodes()) {
      expect(new Set(found.from(node))).toEqual(expected.get(node));
    }
  });
  ...
  it.each(SEEDS)(
    ...
    (seed) => {
      ...
      for (const node of graph.nodes()) {
        expect(new Set(descendants(snapshot, node))).toEqual(
          new Set([...expected.get(node)!].filter((other) => other !== node)),
        );
        for (const other of graph.nodes()) {
          expect(reachable(snapshot, node, other)).toBe(
            node === other || expected.get(node)!.has(other),
          );
        }
      }
      for (const node of graph.nodes()) {
        for (const parent of ancestors(snapshot, node)) {
...
describe("最短路", () => {
  ...
  it.each(SEEDS)(
    ...
    (seed) => {
      ...
      for (const target of graph.nodes()) {
        const best = tree.distance[snapshot.indexOf(target)]!;
        ...
        for (const route of [single!, star!, both!]) {
          ...
          expect(walked(graph, route.path)).toBeCloseTo(best, 9);
        }
      }
    },
  );
    ...
    for (const source of graph.nodes()) {
      ...
      for (const target of graph.nodes()) {
        expect(matrix.at(source, target)).toBeCloseTo(
          tree.distance[snapshot.indexOf(target)]!,
          9,
        );
      }
    }
  ...
  it.each(SEEDS)("seed %i：瓶颈路径的代价等于路径上最重的一段", (seed) => {
    ...
    for (const target of graph.nodes()) {
      ...
      if (tree.distance[t] === Infinity || target === source) continue;
      const route = settle(
        shortestPath(snapshot, source, target, { combine: bottleneck }),
      )!;
      const heaviest = Math.max(
        ...pairs(route.path).map(([a, b]) =>
          Math.min(...graph.between(a, b).map((e) => graph.edgeWeight(e) ?? 1)),
        ),
      );
      expect(heaviest).toBeLessThanOrEqual(tree.distance[t]!);
    }
  });
  ...
  it("提前终止的接口不暴露未收敛的距离", () => {
    ...
    const single = settle(shortestPath(snapshot, nodeId("s"), nodeId("t")))!;
    expect(single.distance).toBe(0);
    expect(single.path).toEqual([nodeId("s"), nodeId("t")]);

    // 全树接口跑完整个搜索，x 必须是收敛后的 2 而不是提前终止时的 5。
    const tree = settle(shortestPaths(snapshot, nodeId("s")));
  ...
  it("负权边让 Dijkstra 抛 Negative，Bellman-Ford 抓出负环", () => {
    ...
    const snapshot = Snapshot.of(graph, { weight: (edge) => edge.weight ?? 1 });
    expect(() => settle(shortestPaths(snapshot, nodeId("a")))).toThrow(
      /negative/,
    );
    expect(() => settle(bellmanFord(snapshot, nodeId("a")))).toThrow(Cycle);
  });
});
...
describe("最小生成森林", () => {
  ...
  it.each(SEEDS)("seed %i：选出的边都真实存在且不成环", (seed) => {
    const graph = randomGraph(seed, { order: 20, density: 2 });
    const links = settle(kruskal(weighted(graph)));
    const seen = new Set<string>();
    for (const link of links) {
      expect(graph.hasEdge(link.edge)).toBe(true);
      expect(edgeKey(graph, link.edge)).toBe(key(link.source, link.target));
      seen.add(key(link.source, link.target));
    }
