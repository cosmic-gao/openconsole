...
```tsx
// app/layout.tsx
import { FontProvider, LayoutProvider, SidebarProvider, ThemeProvider } from "@openconsole/atoms";
...
const account = {
  ...
  menu: [
    ...
    { separator: true, label: "登出", icon: "LogOut", destructive: true, onSelect: () => signOut() },
  ],
};
...
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <>
      <Sidebar brand={{ name: "OpenConsole", logo: "Command" }} menu={menu} account={account} />
      <SidebarInset>
        <Header />
        {children}
      </SidebarInset>
...
粘性顶栏。左右两段：导航段（面包屑 + offcanvas 折叠时的展开按钮）跟随侧边栏所在的一侧；工具段（自定义 actions + 内置 `ThemeSwitch` + 设置按钮）在另一侧。
...
import { Forbidden, NotFound } from "@openconsole/atoms";
...
```tsx
<ColorPicker label="主色" cssVar="--primary" value={current} onChange={(cssVar, value) => apply(cssVar, value)} />
```
...
```tsx
<LayoutProvider defaultConfig={{ variant: "inset", collapsible: "icon", side: "left" }}>...</LayoutProvider>;
