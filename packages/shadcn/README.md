...
`components.json` 的 `@/` 别名与 `tsconfig.json` 的 `paths` **仅供 CLI 解析**。`shadcn add` / `ai-elements add` 生成的文件会带 `@/`，需**手动**按文件位置改回相对路径：`@/lib/utils` → `../../lib/utils`；引用 UI 原语 `@/components/ui/x`（或 `@/registry/default/ui/x`）在 ai-elements 里写 `../ui/x`、在 ui 内部写 `./x`。新增组件别忘了补进对应目录的 `index.ts` barrel。
